...
import { deleteVehicle, fetchVehicles } from "../../redux/vehiclesSlice";
...
const Home = () => {
  ...
  const isLoading = loading || vehiclesLoading || !scenarios || !vehicles;
  const hasError = error || vehiclesError;

  const selectedScenarioData = scenarios.find((s) => s.id === selectedScenario);

  const elements = vehicles
    .filter((v) => v.scenario === selectedScenario)
    .map((v) => ({
      id: v.id,
      ...
      {isLoading ? (
        ...
        <div className="content_wrapper">
          ...
          <table className="table" style={{ marginTop: "1rem" }}>
            ...
            <tbody>
              {vehicles
                .filter((vehicle) => vehicle.scenario === selectedScenario)
                .map((each, i) => (
                  <tr key={i}>
                    <td>{each.id}</td>
                    <td>{each.name}</td>
                    ...
                    <td>
                      <svg
                        width="24"
                        height="24"
                        onClick={() => navigate(`/vehicles/update/${each.id}`)}
                        viewBox="0 0 24 24"
                        fill="none"
                    ...
                    <td>
                      <svg
                        width="21"
                        height="24"
                        onClick={() => dispatch(deleteVehicle({ id: each.id }))}
                        style={{ cursor: "pointer" }}
                        viewBox="0 0 21 24"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
              ...
              .map((each, i) => (
                <div
                  ...
                  style={{
                    ...
                    left: Number(each.positionX),
                    top: Number(each.positionY),
                  }}
                >
                  {i}
                </div>
